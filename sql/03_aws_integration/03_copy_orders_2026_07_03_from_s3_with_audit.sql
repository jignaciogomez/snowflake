-- =============================================================================
-- Step 5.6: Audit-aware orders load from S3
-- Purpose: Load the third orders file from the S3 external stage and audit it.
-- Execution role: NACHO_PLATFORM_ADMIN
-- =============================================================================

EXECUTE IMMEDIATE $$
DECLARE
    v_pipeline_name VARCHAR DEFAULT 'bronze_orders_s3_stage_copy';
    v_source_file_name VARCHAR DEFAULT 'orders_2026-07-03.csv';
    v_source_file_path VARCHAR DEFAULT '@NACHO_DEV_DB.BRONZE.ORDERS_S3_STAGE/dt=2026-07-03/orders_2026-07-03.csv';
...
BEGIN
    ...
    FROM (
        ...
        FROM @NACHO_DEV_DB.BRONZE.ORDERS_S3_STAGE/dt=2026-07-03/orders_2026-07-03.csv
    )
